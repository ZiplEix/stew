...
func RegisterStewRoutes(mux *http.ServeMux) {
    ...
    mux.Handle("GET {{$route.URLPath}}", {{$route.MiddlewareChain}}http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
        ...
        {{$route.LayoutChain}}{{$route.PackageAlias}}.Page(w, data){{$route.LayoutCloseArgs}}
    }){{$route.MiddlewareClose}})
    {{- end}}
    
    {{- if $route.HasServer}}
    {{- range $route.Methods}}
    {{- if ne (toUpper .) "GET" | or (not $route.HasPage)}}
    mux.Handle("{{toUpper .}} {{$route.URLPath}}", {{$route.MiddlewareChain}}http.HandlerFunc({{$route.PackageAlias}}.{{.}}){{$route.MiddlewareClose}})
    {{- end}}
    {{- end}}
    {{- end}}
    {{- end}}
...
func (w *Writer) buildLayoutChain(n *RouteNode) (string, string) {
	...
	for curr != nil {
		if curr.HasLayout {
			layouts = append([]string{curr.PackageAlias + ".Layout(w, data, func() {\n"}, layouts...)
			closeArgs = append(closeArgs, "\n})")
		}
		curr = curr.Parent
	}
